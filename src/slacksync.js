...
async function slacksync(opts) {
  ...
  try {
    const excludeUsers = opts.exclude_maillist
      ? (await getMaillistMembers(
          opts.exclude_maillist,
          opts.maillist_token
        )).map(u => `${u.username}@sfu.ca`)
      : [];
    ...
    const deletedSlackUsers = allSlackUsers.members.filter(
    ...
    maillistMembers = maillistMembers
      .map((members, idx) => {
        ...
        return maillistType === 1
          ? members.filter(u => u.autoGenerated)
          : members;
      })
    ...
    const usersToRemoveFromSlack = activeSlackUsers.filter(
      u =>
    ...
    if (usersToRemoveFromSlack.length > 0 && opts.remove_users) {
      results.removed = await toggleUsersInSlack(
        usersToRemoveFromSlack,
        false,
        opts
      );
    }

    const usersToReactivate = deletedSlackUsers.filter(
      u =>
        maillistMembersAddresses.includes(u.profile.email) &&
        !IGNORE_USERS.includes(u.profile.email.split('@')[0]) &&
        !excludeUsers.includes(u.profile.email)
    ...
    if (usersToReactivate.length > 0 && opts.reactivate_users) {
      results.reactivated = await toggleUsersInSlack(
        usersToReactivate,
        true,
        opts
      );
    ...
    if (
      ...
      (results.created || results.removed || results.reactivated)
    ) {
...
async function toggleUsersInSlack(users, state, opts) {
  ...
  const targets = userIds.map(id =>
    slack.toggleUserActive(id, state, opts.slack_token)
  );
  let responses = [];
